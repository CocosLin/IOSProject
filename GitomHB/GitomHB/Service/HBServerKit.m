                                          ...
                                          andInTime3:(int)inTime3
{
    ...
    if (outTime1>1) [attenWorktimeArr addObject:attenWorktimeDic1];
    if (outTime2>1) [attenWorktimeArr addObject:attenWorktimeDic2];
...
- (void)applyJoinToCompanyWithOrganizationId:(NSString *)organizationId
    ...
    NSString *urlStr = [NSString stringWithFormat:@"http://hb.m.gitom.com/3.0/user/saveApply?organizationId=%@&orgunitId=%@&note=%@&username=%@&cookie=%@&verifyType=%@",organizationId,orgunitId,note,username,_cookie,[verifyType stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
    NSURL *url = [NSURL URLWithString:urlStr];
    ...
    [req setCompletionBlock:^{
        ...
        if (body.success)
        {
            ...
            if (getArr.count) {
                ...
                for (int i=0; i<getArr.count; i++) {
                    NSLog(@"HBServerKit getArr Count == %d",getArr.count);
