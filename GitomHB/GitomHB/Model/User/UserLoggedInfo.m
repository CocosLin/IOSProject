...
-(id)initForAllJsonDataTypeWithDicFromJson:(NSDictionary *)dicFromJson
{
    if (self = [super initForAllJsonDataTypeWithDicFromJson:dicFromJson]) {
        ...
        NSArray *orgPropsArray = [dicFromJson objectForKey:@"orgunitProps"];
        
        if (orgPropsArray.count>0) {
            self.checkWay = [[orgPropsArray objectAtIndex:0]objectForKey:@"propValue"];
            NSLog(@"UserLonggedInfo propValue = %@",self.checkWay);
            if (orgPropsArray.count>1){
                self.checkWay = [[orgPropsArray objectAtIndex:1]objectForKey:@"propValue"];
                NSLog(@"UserLonggedInfo propValue == %@",self.checkWay);
            }
            if (orgPropsArray.count>2){
                self.checkWay = [[orgPropsArray objectAtIndex:2]objectForKey:@"propValue"];
                NSLog(@"UserLonggedInfo propValue = %@",self.checkWay);
            }
        }
        
        if ([self.checkWay isEqual:NEED_QUESTION]) {
            self.question = [[orgPropsArray objectAtIndex:3]objectForKey:@"propValue"];
            self.answers = [[orgPropsArray objectAtIndex:1]objectForKey:@"propValue"];
        }else{
            self.question = @"";
            self.answers = @"";
        }
