...
- (IBAction)playOriginWavBtnPressed:(id)sender {
    if (originWav.length > 0) {
        ...
        NSLog(@"播放原wav %@",[VoiceRecorderBaseVC getPathByFileName:originWav ofType:@"wav"]);
        [player play];
    }
}
#pragma mark - wav转amr
- (IBAction)wavToAmrBtnPressed:(id)sender {
    NSLog(@"对路径中 %@ 由wav转amr ",originWav);
    if (originWav.length > 0){
        ...
        NSLog(@"%@ 转换后的amr路径 %@",[VoiceRecorderBaseVC getPathByFileName:originWav ofType:@"wav"],[VoiceRecorderBaseVC getPathByFileName:convertAmr ofType:@"amr"]);
