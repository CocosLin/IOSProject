...
{
    UserLoggingInfo * userLoggingInfo = loggingInfo; //保存一份loggingInfo
 
    //验证用户名和密码输入是否有问题
    NSLog(@"UserManager == 登录");
    ...
    if (!strError) {
        ...
        [_serverKit loggingWithUsername:userLoggingInfo.username
                        Md5PasswordUp:userLoggingInfo.password
                          VersionCode:@"9999"
                           GotJsonDic:^(NSDictionary * dicUserLogged, WError *myError)
         {
             ...
             {
                 UserLoggedInfo * loggedInfo = [[UserLoggedInfo alloc]initForAllJsonDataTypeWithDicFromJson:dicUserLogged];//获得cookie 、
