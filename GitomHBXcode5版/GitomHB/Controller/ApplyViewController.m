...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *applynameStr = [[[applyMod.note componentsSeparatedByString:@"("]objectAtIndex:0] stringByReplacingOccurrencesOfString:@"[" withString:@""];
    NSLog(@"ios 5 + applyName == %@",applynameStr);
    myCell.nameLabel.text = [NSString stringWithFormat:@"%@(%@)申请加入[%@]",applynameStr,applyMod.createUserId,applyMod.orgunitName];
    ...
    NSString *resonStr = [[applyMod.note componentsSeparatedByString:@"]"] lastObject];
    
    myCell.addressLabel.text = [NSString stringWithFormat:@"申请理由:%@",resonStr];
    ...
    /*
        ...
        applyMod = [self.arrData objectAtIndex:indexPath.row];
        NSString *applynameStr = [[[applyMod.note componentsSeparatedByString:@"("]objectAtIndex:0] stringByReplacingOccurrencesOfString:@"[" withString:@""];
        NSLog(@"ios 6 + applyName == %@",applynameStr);
        myCell.realName.text = [NSString stringWithFormat:@"%@(%@)申请加入[%@]",applynameStr,applyMod.createUserId,applyMod.orgunitName];
        ...
        NSString *resonStr = [[applyMod.note componentsSeparatedByString:@"]"] lastObject];
        myCell.address.text = [NSString stringWithFormat:@"申请理由:%@",resonStr];
        myCell.backgroundView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"cell_bg.png"]];
        myCell.selectedBackgroundView=[[UIImageView alloc]initWithImage:[UIImage imageNamed:@"cell_bg_press.png"]];
        return myCell;
    }*/
...
- (void) alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    NSLog(@"UIAlerView delegate math");
    GetCommonDataModel;
    applyMod = [self.arrData objectAtIndex:selectedIndex];
    NSLog(@"ApplyViewController applyMod.note == %@",applyMod.note);
    NSString *applynameStr = [[[applyMod.note componentsSeparatedByString:@"("]objectAtIndex:0] stringByReplacingOccurrencesOfString:@"[" withString:@""];
    NSLog(@"ApplyViewController applynameStr == %@",applynameStr);
    NSString *phoneStr = [[[[applyMod.note componentsSeparatedByString:@"("]objectAtIndex:1] componentsSeparatedByString:@")"]objectAtIndex:0];
    switch (buttonIndex) {
        case 0:
            //同意申请http://hb.m.gitom.com/3.0/organization/saveOrgunitUser?organizationId=114&orgunitId=1&username=90261&roleId=4&updateUser=90261&cookie=5533098A-43F1-4AFC-8641-E64875461345 &telephone=111111&realname=xxxyyy
        {
            NSString *saveOrgunitUserStr = [NSString stringWithFormat:@"http://hb.m.gitom.com/3.0/organization/saveOrgunitUser?organizationId=%ld&orgunitId=%@&roleId=%@&username=%@&updateUser=%@&cookie=%@&operations=8&telephone=%@&realname=%@",(long)comData.organization.organizationId,applyMod.orgunitId,@"4",applyMod.createUserId,comData.userModel.username,comData.cookie,phoneStr,[applynameStr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
            ...
            if (bo) {
                ...
                [SVProgressHUD showSuccessWithStatus:[NSString stringWithFormat:@"%@%@",applynameStr,warningStr]];
            ...
            NSString *changeRoleUrlStr = [NSString stringWithFormat:@"http://hb.m.gitom.com/3.0/organization/deleteApply?organizationId=%ld&orgunitId=%@&username=%@&updateUser=%@&cookie=%@",(long)comData.organization.organizationId,applyMod.orgunitId,applyMod.createUserId,comData.userModel.username,comData.cookie];
            ...
            [SVProgressHUD showSuccessWithStatus:@"拒绝申请"];
            break;
        }case 2:{
            return;
        }
