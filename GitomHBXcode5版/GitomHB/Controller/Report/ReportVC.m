...
- (void)sendFileToServe{
    ...
    for (int i =0; i<4; i++) {
        ...
        if ([NSData dataWithContentsOfFile:photoPath]) {
...
- (void)useImage:(UIImage *)image {
    ...
    [imgViewPhoto1 setImage:newImage];
...
-(void)btnBack:(UIButton *)btn
{
    ...
    if ([comData.organization.appLevelCode isEqualToString:kAdvance]) {
        ...
        NSLog(@"ReiportVC accuracy == %f  address == %@",accuracy,self.myAddress);
        
        [hbKit sendUserPositionWithaccuracy:accuracy
                                andlatitude:self.latitude
                                andlocation:self.myAddress
    ...
    for (int i =0; i<4; i++) {
        NSString *photoPath = [NSString stringWithFormat:@"%@%@",NSTemporaryDirectory(),[NSString stringWithFormat:@"myPhoto%d.jpg",3010+i]];
        NSData *hasphotoData = [NSData dataWithContentsOfFile:photoPath];
        if (hasphotoData) {
...
- (void)viewDidLoad
{
    ...
    [btnSaveReoprt setTitle:@"放开上传……" forState:UIControlStateHighlighted];
