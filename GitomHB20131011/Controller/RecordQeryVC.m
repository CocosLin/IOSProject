...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    switch (buttonIndex)
    {
        ...
        case 1://工作汇报查询
        {
            ...
            [hbServer findOrgunitReportsOfMembersWithOrganizationId:[orgMod.organizationId integerValue]    orgunitId:[orgMod.orgunitId integerValue] ReportType:@"REPORT_TYPE_DAY_REPORT" BeginDateLli:nil EndDateLli:nil FirstReportRecord:nil MaxReportRecord:nil GotArrReports:^(NSArray *arrDicReports, WError *myError)
             {
        ...
        case 2://外出汇报查询
        {
            ...
            [SVProgressHUD showWithStatus:@"加载工作汇报…"];
            HBServerKit *hbServer = [[HBServerKit alloc]init];
            [hbServer findOrgunitReportsOfMembersWithOrganizationId:[orgMod.organizationId integerValue] orgunitId:[orgMod.orgunitId integerValue]ReportType:@"REPORT_TYPE_GO_OUT" BeginDateLli:nil EndDateLli:nil FirstReportRecord:nil MaxReportRecord:nil GotArrReports:^(NSArray *arrDicReports, WError *myError)
             {
                 if (arrDicReports.count) {
                     RecordQeryReportsVC *rdVC= [[RecordQeryReportsVC alloc]init];
                     rdVC.arrData = arrDicReports;
                     rdVC.strTypeRecord = @"外出汇报";
                     rdVC.userRealname = orgMod.organizationName;
                     rdVC.title = [NSString stringWithFormat:@"%@记录",orgMod.organizationName];
                     [SVProgressHUD dismissWithSuccess:@"加载成功"];
                     [self.navigationController pushViewController:rdVC animated:YES];
        ...
        case 3://出差汇报查询
        {
            ...
            [SVProgressHUD showWithStatus:@"加载工作汇报…"];
            HBServerKit *hbServer = [[HBServerKit alloc]init];
            [hbServer findOrgunitReportsOfMembersWithOrganizationId:[orgMod.organizationId integerValue]     orgunitId:[orgMod.orgunitId integerValue] ReportType:@"REPORT_TYPE_TRAVEL" BeginDateLli:nil EndDateLli:nil FirstReportRecord:nil MaxReportRecord:nil GotArrReports:^(NSArray *arrDicReports, WError *myError)
             {
                 if (arrDicReports.count) {
                     RecordQeryReportsVC *rdVC= [[RecordQeryReportsVC alloc]init];
                     rdVC.arrData = arrDicReports;
                     rdVC.strTypeRecord = @"出差汇报";
                     rdVC.userRealname = orgMod.organizationName;
                     rdVC.title = [NSString stringWithFormat:@"%@记录",orgMod.organizationName];
