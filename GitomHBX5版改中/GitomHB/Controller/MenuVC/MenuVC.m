        ...
        if (_currentSeletedMenuFlag == 2)
        {
            ...
            if (_currentSeletedMenuFlag == 3)
            {
                if (indexPath.row==0)//分享应用
                {
                    ...
                    [UMSocialData defaultData].extConfig.title = @"我在用移动汇报\nhttp://app.gitom.com";
                    [[UMSocialData defaultData].extConfig setAppUrl:@"http://app.gitom.com/MobileApp/detail/12"];
                    [UMSocialSnsService presentSnsController:self
                                                         appKey:@"52ac1e8556240b08a00c42bc"
                                                      shareText:@"我正在使用移动汇报http://app.gitom.com/mobileapp/list/12"
        ...
        case 0:
        {
            if (tf.text.length > 0) {
                ...
                if ([hbKit isNumbers:tf.text]) {
                    ...
                    [SVProgressHUD showErrorWithStatus:@"命名不能含网即通字样！" duration:1.6];
