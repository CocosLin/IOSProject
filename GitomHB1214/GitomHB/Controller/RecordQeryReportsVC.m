...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([[[UIDevice currentDevice] systemVersion] floatValue]<6.0) {
        ...
        if (self.playCard==YES) {
            ...
            NSLog(@"RecordQeryReportsVC cell == %@ %@",[WTool getStrDateTimeWithDateTimeMS:_atteMod.updateDate DateTimeStyle:@"yyyy-MM-dd"],[WTool getStrDateTimeWithDateTimeMS:_atteMod.createTime DateTimeStyle:@"HH:mm:ss"]);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    GetCommonDataModel;
    dvc.title = [NSString stringWithFormat:@"%@详情",recordType];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (self.playCard == YES) {
        ...
        [SVProgressHUD showWithStatus:@"加…载"];
                                                         ...
                                                         GotArrReports:^(NSArray *arrDicReports, WError *myError)
             {
                 if (arrDicReports.count) {
                     ...
                     for (NSDictionary * dicReports in arrDicReports)
                     {
                         ...
                         NSLog(@"RecordQeryReportsVC realname == %@",dicReports);
                         ...
                         NSLog(@"RecordQeryReportsVC note = %@",repMod.note);
...
-(void)initRecordPromptInfo
{
    ...
    if (self.playCard == YES) {
        ...
        recordType = [[NSString alloc]init];
        if ([self.strTypeRecord isEqualToString:REPORT_TYPE_DAY_REPORT]) {
            recordType = @"工作汇报";
        }else if ([self.strTypeRecord isEqualToString:REPORT_TYPE_GO_OUT]) {
            recordType = @"外出汇报";
        } else if ([self.strTypeRecord isEqualToString:REPORT_TYPE_TRAVEL]){
            recordType = @"出差汇报";
        }
