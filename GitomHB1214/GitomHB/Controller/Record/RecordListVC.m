...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([[[UIDevice currentDevice]systemName] floatValue]<6.0) {
        ...
        if (!self.typeRecord) {
            ...
            myCell.timeLabel.text = [NSString stringWithFormat:@"%@ %@",[WTool getStrDateTimeWithDateTimeMS:[[dicReports objectForKey:@"createDate"]floatValue] DateTimeStyle:@"yyyy-MM-dd"],[WTool getStrDateTimeWithDateTimeMS:[[dicReports objectForKey:@"createTime"]floatValue] DateTimeStyle:@"HH:mm:ss"]];
            ...
            NSLog(@"RecordListVC Cell self.arrData == %@",self.arrData);
        ...
        if (!self.typeRecord) {
            ...
            myCell.creatDate.text = [NSString stringWithFormat:@"%@ %@",[WTool getStrDateTimeWithDateTimeMS:[[dicReports objectForKey:@"createDate"]floatValue] DateTimeStyle:@"yyyy-MM-dd"],[WTool getStrDateTimeWithDateTimeMS:[[dicReports objectForKey:@"createTime"]floatValue] DateTimeStyle:@"HH:mm:ss"]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.typeRecord) {
        ...
        dvc.title = [NSString stringWithFormat:@"%@详情",_strRecordType];
        GetCommonDataModel;
...
//        }
    ...
    {
        [SVProgressHUD showWithStatus:@"加载记录…"];
        ...
        }else//汇报记录
    {
        ...
        [rm findReportsWithOrganizationId:comData.organization.organizationId
                            ...
                            GotArrReports:^(NSArray *arrReports, BOOL isOk)
         {
             if (arrReports.count) {
                 self.arrData = arrReports;
                 NSLog(@"RecordListVC arrReports == %@",arrReports);
